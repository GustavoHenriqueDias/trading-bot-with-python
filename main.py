...
class MultidimensionalTransdimensionalSplitter(QCAlgorithm):
    ...
    def EveryMarketOpen(self):
        ...
        self.lookback = self.lockback * (1 + deltavol)
        ...
        if not self.Securities[self.symbol].Invested and self.Securities[self.symbol].Close >= max(self.high[:-1]):
            ...
            self.breackoutlvl = max(self.high[:-1])
            self.highestPrice = self.breackoutlvl   
        
        if self.Securities[self.symbol].Invested:
            if not self.Transactions.GetOpenOrders(self.symbol):
                self.stopMarketTicket = self.StopMarketOrder(self.symbol, -self.Portfolio[self.symbol].Quantity, self.initialStopRisk * self.breackoutlvl)

            if self.Securities[self.symbol].Close > self.highestPrice and self.initialStopRisk * self.breackoutlvl < self.Securities[self.symbol].Close * self.trailingStopRisk:
